...
import com.gitlab.essobedo.appma.core.impl.config.ConfigFromProperties;
import com.gitlab.essobedo.appma.core.impl.config.ConfigurationFactory;
import com.gitlab.essobedo.appma.core.impl.io.Folder;
import com.gitlab.essobedo.appma.core.impl.progress.LogProgress;
import com.gitlab.essobedo.appma.core.impl.progress.StatusBar;
import com.gitlab.essobedo.appma.core.impl.zip.UnzipTask;
...
public class DefaultApplicationManager implements ApplicationManager {
    ...
    private void reload(final Configuration configuration) throws ApplicationException {
        if (configuration == null) {
            ...
            try {
                ...
                LOG.log(Level.ALL, "The configuration could not be stored", e);
                loadConfiguration();
            }
        }
    }
    ...
    protected Manageable create() throws ApplicationException {
        ...
        while (iterator.hasNext()) {
            ...
            if (LOG.isLoggable(Level.FINE)) {
                LOG.log(Level.FINE, String.format("Found application '%s' version '%s' has ben found", app.name(),
                    app.version()));
            }
    ...
    public Task checkForUpdate() {
        ...
        final ServiceLoader<VersionManager> loader = ServiceLoader.load(VersionManager.class,
        ...
        try {
            ...
            return iterator.next().check(application);
        } catch (ApplicationException e) {
            if (LOG.isLoggable(Level.SEVERE)) {
    ...
    void doUpgrade() throws ApplicationException {
        ...
        if (patchFolder == null || !applyPatch(patchFolder, oldVersion)) {
            return;
        }
    ...
    private boolean applyPatch(final File patchFolder, final String oldVersion) throws ApplicationException {
        final ClassLoader contextCL = Thread.currentThread().getContextClassLoader();
        try {
            ...
            final ServiceLoader<VersionManager> loader = ServiceLoader.load(VersionManager.class, cl);
            final Iterator<VersionManager> iterator = loader.iterator();
            if (!iterator.hasNext()) {
                throw new ApplicationException("No version manager could be found");
            ...
            final Configuration configuration = executeTask(iterator.next().upgrade(patchFolder, root, oldVersion));
